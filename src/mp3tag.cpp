...
ReadMp3Tag::~ReadMp3Tag ( void ) {
    if ( techinfo != NULL ) { delete [] techinfo; techinfo= NULL; }
    if ( Year     != NULL ) { delete [] Year;     Year    = NULL; }
    if ( Album    != NULL ) { delete [] Album;    Album   = NULL; }
    if ( Artist   != NULL ) { delete [] Artist;   Artist  = NULL; }
...
int resync_mp3 ( FILE *fp,int pos ) {
    ...
    for ( i=0;i<65536;i++ ) {
        ...
        if ( feof ( fp ) ) return -1;
        if ( ( Gfsize=head_check2 ( head ) ) ) return pos+i;
    }
