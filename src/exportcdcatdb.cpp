...
void exportCdcatDB::fillMedia() {
    ...
    setWindowTitle( tr( "Export %1 database: " ).arg(PROGRAM_NAME) + ((DBCatalog *)(source->data))->name );
    Node *tmp;
    tmp = source->child;
...
void exportCdcatDB::ok() {
    if ((fileName->text()).isEmpty()) {
        ...
        if (radioHtml->isChecked()) {
            ...
            outstring += "\n";
            outstring += " <!-- This file was generated by " PROGRAM_NAME;
            outstring += HOMEPAGE;
            outstring += " -->\n";
            outstring += "<title> " + tr( "Export from %1:" ).arg(PROGRAM_NAME) + " " + ((DBCatalog *)((p->getRootNode())->data))->name + "</title>\n";
            ...
            outstring += "table { border-collapse: collapse; }"; // extremely useful table feature,
            // this causes table border to collapse into 1px wide border
            // and there is no need 4 putting "&nbsp;" instead " " (space) to display empty cells correctly
            ...
            if (checkExportTitle->isChecked()) {
                outstring += "<h1>" + tr( "Export from %1, catalog:" ).arg(PROGRAM_NAME) + " " + ((DBCatalog *)((p->getRootNode())->data))->name + "</h1>\n";
            }
            ...
            if (checkExportTableHeader->isChecked()) {
                outstring += "# " + tr( "Export from %1, catalog:" ).arg(PROGRAM_NAME) + " " + ((DBCatalog *)((p->getRootNode())->data))->name + "\n";
        ...
        if (checkExportMp3Tag->isChecked()) {
            ...
            while (tmp != NULL) {
                ...
                if (tmp->type == HC_MP3TAG) {
                    qDebug() << "export: artist: mp3tag found (1)";
