 ...
 */
exportCdcatDB::exportCdcatDB ( QApplication *appp,DataBase *db, QWidget* parent, const char* name, bool modal, Qt::WFlags fl ) : QDialog ( parent, name, modal, fl ) {
    p = db;
    app = appp;
    pww = NULL;
    ...
    if ( p != NULL )
        fileName->setText ( QString ( getenv ( "HOME" ) ) +"/"+ ( ( DBCatalog * ) ( ( p->getRootNode() )->data ) )->name+".html" );
#endif
    ...
    if ( ( fileName->text() ).isEmpty() )
        ...
        if ( !f.exists() || overwrite == QMessageBox::Yes ) {
            ...
            if ( f.open ( QIODevice::WriteOnly ) ) {
                ...
                //QApplication::restoreOverrideCursor();
                QMessageBox::critical ( 0, tr ( "Error during write config..." ), tr ( "I can't create or rewrite the file" ) );
            }
            close();
        } else {
            //QApplication::restoreOverrideCursor();
...
void exportCdcatDB::exportTypeToggled() {
    ...
    if ( radioCsv->isChecked() ) {
        ...
        if ( p != NULL )
            fileName->setText ( QString ( getenv ( "HOME" ) ) +"/"+ ( ( DBCatalog * ) ( ( p->getRootNode() )->data ) )->name+".csv" );
#endif
